...
function formatTime(minutes, seconds) {
  const minutFormated = String(minutes).padStart(2, '0');
  const secFormated = String(seconds).padStart(2, '0');
  return `${minutFormated}:${secFormated}`;
}
/**
...
function reverseWords(str) {
  ...
  for (let i = 0; i < newArr.length; i += 1) {
    tempStr = newArr[i].split('').reverse().join('');
    reversStr += `${tempStr} `;
  }
...
function extractNameFromTemplate(value) {
  const newStr = value.replace('Hello, ', '').slice(0, -1);
  return newStr;
}
